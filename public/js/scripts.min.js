...
return this.each(function(){var index,data=data_priv.get(this),queue=data[type+"queue"],hooks=data[type+"queueHooks"],timers=jQuery.timers,length=queue?queue.length:0;data.finish=true;jQuery.queue(this,type,[]);if(hooks&&hooks.cur&&hooks.cur.finish){hooks.cur.finish.call(this);}
...
if(typeof window==="object"&&typeof window.document==="object"){window.jQuery=window.$=jQuery;}})(window);(function(){var root=this;var previousUnderscore=root._;var breaker={};var ArrayProto=Array.prototype,ObjProto=Object.prototype,FuncProto=Function.prototype;var push=ArrayProto.push,slice=ArrayProto.slice,concat=ArrayProto.concat,toString=ObjProto.toString,hasOwnProperty=ObjProto.hasOwnProperty;var
...
_.extend(child,parent,staticProps);var Surrogate=function(){this.constructor=child;};Surrogate.prototype=parent.prototype;child.prototype=new Surrogate;if(protoProps)_.extend(child.prototype,protoProps);child.__super__=parent.prototype;return child;};Model.extend=Collection.extend=Router.extend=View.extend=History.extend=extend;var urlError=function(){throw new Error('A "url" property or function must be specified');};var wrapError=function(model,options){var error=options.error;options.error=function(resp){if(error)error(model,resp,options);model.trigger('error',model,resp,options);};};}).call(this);(function()
{window.App={Models:{},Collections:{},Views:{},Router:{},Helpers:{}};window.vent=_.extend({},Backbone.Events);})();_.templateSettings.interpolate=/\{\{(.*?)\}\}/g;var Templates={};Templates.Task=['<div class="check-complete">','<input type="checkbox" id="complete-{{ title }}" class="complete" {{ checked }}>','<label for="complete-{{ title }}"></label>','</div>','<div class="task-content">','<div class="task-text">','<span class="title">{{ title }}</span>','<input class="edit-field" type="text" value="{{ title }}" onfocus="this.value = this.value;">','<a href="#" class="delete">delete</a>','</div>','<div class="details clearfix">','<span class="time">{{ reltime }} ago</span>','<div class="actions">','<span class="close">close</span>','<span class="edit">edit</span>','<span class="complete">complete</span>','</div>','</div>','</div>',].join('');for(var temp in Templates)
{if(Templates.hasOwnProperty(temp))
{Templates[temp]=_.template(Templates[temp]);}};App.Helpers.RelativeTime=function(date){var seconds=Math.floor((new Date().getTime()-Date.parse(date))/1000);var interval=Math.floor(seconds/31536000);if(interval>1){return interval+"y";}
interval=Math.floor(seconds/604800);if(interval>1){return interval+"w";}
interval=Math.floor(seconds/86400);if(interval>1){return interval+"d";}
...
App.Models.Task=Backbone.Model.extend({defaults:{title:'',completed:0},urlRoot:'/api/lists/'+list+'/tasks'});App.Collections.Tasks=Backbone.Collection.extend({model:App.Models.Task,url:'/api/lists/'+list+'/tasks'});App.Router=Backbone.Router.extend({routes:{'all':'showAll','completed':'showComplete','uncompleted':'showUncomplete'},showAll:function()
{this.view.filterType='all';this.view.trigger("change:filterType");},showComplete:function()
{this.view.filterType='completed';this.view.trigger("change:filterType");},showUncomplete:function()
{this.view.filterType='uncompleted';this.view.trigger("change:filterType");}});App.Views.Tasks=Backbone.View.extend({el:$('.tasks-list'),initialize:function()
{if(this.filterType===undefined)
{var that=this;this.collection.fetch().then(function()
{that.render();});}
this.collection.on('add',this.add,this);this.collection.on('remove',this.renderCount,this);this.collection.on('reset',this.render,this);this.on("change:filterType",this.filterByCompleted,this);},render:function()
{this.$el.empty();this.$el.append('<span class="tasks-length">'+this.collection.length+' tasks in this list.</span>');this.collection.each(this.add,this);return this;},add:function(task)
{var taskView=new App.Views.Task({model:task});this.$el.append(taskView.render().el);this.renderCount();},renderCount:function()
{this.$el.find('.tasks-length').html(this.collection.length+' tasks in this list.');},filterByCompleted:function()
{var tasks=new App.Collections.Tasks(),filterType=this.filterType,collection=this.collection;tasks.fetch().then(function()
{if(filterType==='all')
{collection.reset(tasks.models);}else
{var completed='0';if(filterType==='completed')
{completed='1';}
collection.reset(tasks.where({completed:completed}));}});}});App.Views.Task=Backbone.View.extend({tagName:'li',events:{'click .task-text':'showDetails','click .close':'hideDetails','click .edit':'edit','click .complete':'complete','click .title':'edit','blur .edit-field':"close",'click .complete':'complete','click .delete':'delete'},template:Templates.Task,initialize:function(){this.listenTo(this.model,'change',this.render);},render:function()
{var data=this.model.toJSON();var checked='';if(this.model.get('completed')==1)
checked='checked';data.checked=checked;data.reltime=App.Helpers.RelativeTime(data.created_at);this.el.innerHTML=this.template(data);this.input=this.$('.edit-field');return this;},edit:function()
{this.$el.addClass("editing");this.input.focus();},close:function()
{this.$el.removeClass("editing");this.model.set('title',this.input.val());this.model.save();},complete:function()
{if(this.model.get('completed')==0)
this.model.set('completed',1);else
this.model.set('completed',0);this.model.save();},delete:function(e)
{e.preventDefault();this.model.destroy();this.$el.remove();},showDetails:function()
{$('.open').removeClass('open');this.$el.addClass('open');},hideDetails:function()
{this.$el.removeClass('open');}});App.Views.CreateTask=Backbone.View.extend({el:'#createTask',events:{'submit':'create'},create:function(e)
{e.preventDefault();var input=this.$el.find('#taskContent');var title=input.val();input.val('');var task=new App.Models.Task({title:title});var that=this;task.save().then(function()
{that.collection.add(task);});}});var AppView=Backbone.View.extend({initialize:function()
{var tasks=new App.Collections.Tasks(),createTask=new App.Views.CreateTask({collection:tasks}),tasksView=new App.Views.Tasks({collection:tasks});var router=new App.Router();router.view=tasksView;Backbone.history.start();}});